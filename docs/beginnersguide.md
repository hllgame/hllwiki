...
### Infantry Roles
{{ icon('rifleman') }}**Rifleman** – Standard infantry with a rifle. Excellent way to learn the game.

{{ icon('assault') }}**Assault** – Close-quarters specialist with SMG or shotgun.

{{ icon('support') }}**Support** – Carries ammo boxes and {{ icon('supplies') }}supplies for building; vital for {{ icon('officer') }}officers to build {{ icon('garrison') }}garrisons.

{{ icon('medic') }}**Medic** – Revives and heals downed teammates.

{{ icon('automaticrifleman') }}**Automatic Rifleman** – Carry automatic rifles; bridges gap between Rifleman and MG.

{{ icon('machinegunner') }}**Machine Gunner** – Lays down heavy suppressive fire from distance; excels at locking down lanes and choke points.
...
- {{ icon('hq') }}**HQ**: 3 spawn points edge of the map you can redeploy at.
- {{ icon('garrison') }}**Garrison**: A forward spawn point built by a {{ icon('officer') }}officer/{{ icon('commander') }}commander, anyone on the **team** can respawn on it.
- {{ icon('outpost') }}**Outpost**: A spawn point built by **your {{ icon('officer') }}officer**, only **your squad** can respawn on it.
- {{ icon('airhead') }}**Airhead**: Deployed by {{ icon('commander') }}commander, anyone on the **team** can respawn on it.

<br>

![](beginnersguide/beginnersguide_image3.png)

- If a spawn point shows up with a {{ icon('ping') }}ping icon, it means enemy is nearby. Caution is advised.
...
- Each teams has **3 {{ icon('hq') }}HQ spawns** available with a respawn timer of 10 seconds at the edge of the map.
...
- {{ icon('garrison') }}Garrisons, {{ icon('outpost') }}Outpost (OPs), {{ icon('halftrack') }}Half-tracks, and {{ icon('airhead') }}Airheads can be used as additional respawn points.
- **{{ icon('garrison') }}Garrisons and {{ icon('outpost') }}Outposts are immediately lost in the sectors that gets captured by enemy**, new ones must be built.
...
- **Attackers only get one {{ icon('hq') }}HQ spawn in the beginning**, they must work to build additional spawn points.
- Defenders have 3 {{ icon('hq') }}HQ spawn points plus an additional **3 {{ icon('garrison') }}garrisons at the frontline** at the beginning of round.
